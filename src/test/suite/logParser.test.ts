...
suite('LogParser Unit Tests', () => {
    ...
    suite('Basic Format Parsing', () => {
        test('Should parse basic log line with script and line metadata', () => {
            ...
            assert.strictEqual(event!.metadata?.script, '/home/testus/wincc_proj/DevEnv/scripts/openParaListener.ctl');
            assert.strictEqual(event!.metadata?.line, 8);
        });

        test('Should parse identifier without spaces before parentheses', () => {
            const line = 'WCCILdataSQLite(0), 2025.12.13 15:23:45.123, SQL, ERROR, 42, Connection failed';
        ...
        test('Should parse scope and severity without space after comma', () => {
            const line = 'WCCOActrl    (1), 2025.12.14 10:00:00.000, PARAM,WARNING, 100, Configuration issue detected';
    ...
    suite('Metadata Line Parsing', () => {
        test('Should parse comma-prefixed line metadata', () => {
            ...
            assert.strictEqual(event!.metadata?.library, '/home/testus/wincc_proj/DevEnv/scripts/HelloWorld.ctl');
        ...
        test('Should parse multi-line error with stacktrace', () => {
            ...
            assert.strictEqual(event!.metadata?.stacktrace![0].filePath, '/opt/WinCC_OA/libs/dataLib.ctl');
            ...
            assert.strictEqual(event!.metadata?.stacktrace![1].filePath, '/opt/WinCC_OA/scripts/main.ctl');
    ...
    suite('Severity Normalization', () => {
        test('Should normalize ERROR to FATAL', () => {
            const line = 'WCCOActrl    (1), 2025.12.14 09:00:00.000, CTRL, ERROR, 500, Critical system failure';
        ...
        test('Should keep WARNING as WARNING', () => {
            const line = 'WCCOActrl    (1), 2025.12.14 09:00:00.000, CTRL, WARNING, 100, Minor issue';
        ...
        test('Should keep INFO as INFO', () => {
            const line = 'WCCOActrl    (1), 2025.12.14 09:00:00.000, CTRL, INFO, 10, Information message';
        ...
        test('Should keep SEVERE as SEVERE', () => {
            const line = 'WCCOActrl    (1), 2025.12.14 09:00:00.000, CTRL, SEVERE, 300, Severe error';
    ...
    suite('Edge Cases', () => {
        ...
        test('Should handle trailing comma in message', () => {
            const line = 'WCCOActrl    (1), 2025.12.14 10:00:00.000, CTRL, WARNING, 76, Message with trailing comma,';
        ...
        test('Should handle message number with slash notation', () => {
            const line = 'WCCOActrl    (2), 2025.12.13 21:12:36.581, PARAM, WARNING, 3/ctrl, Library not found';
